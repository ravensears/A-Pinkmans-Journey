...
class Game extends Phaser.Scene {
  ...
  create() {
    ...
    this.timerText = this.add
      .text(500, 10, "Countdown: " + formatTime(this.initialTime))
      .setScrollFactor(0);
  ...
  update() {
    this.treasureDetector = () => {
      const treasureProximity = (distance) => {
        return (
          Math.abs(
            this.hero.x - this.treasureGroup[this.treasureIndex - 1].x
          ) <= distance &&
          Math.abs(
            this.hero.y - this.treasureGroup[this.treasureIndex - 1].y
          ) <= distance
        );
      };
